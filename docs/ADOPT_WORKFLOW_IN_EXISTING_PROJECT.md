...
如果还不能运行，记录第一个关键错误和已排除原因，当前状态是 `BLOCKED_RUNTIME`，不是“接入完成”。历史检索无命中或候选无法区分时，按[知识复用教程的无死路分支](FIRST_KNOWLEDGE_REUSE_DRILL.md#8-无命中或无法区分时不要重新开始)把原始查询、已开候选、负面发现、权限和一个具体问题交给唯一所有者；不要重新发送一句“跑不起来”。需要向第三方项目反馈时，再按[第一次向开源上游求助与贡献](UPSTREAM_HELP_AND_CONTRIBUTION.md)确认支持范围、问题所有者和公开最小复现；论文结果不同不自动等于上游软件 bug。
...
- 同一失败、复现差异或决定第二次被查询：在现有 Wiki / tracker 增加知识条目和薄索引；
...
- [ ] 可复用经验只链接主要来源，并写清检索词、范围、状态和替代关系；
- [ ] 第一次使用历史经验时保留原始症状查询，比较至少两个候选，并核对当前身份、状态、来源和副作用；
- [ ] 检索无命中或无法区分时，自助上下文进入同一调试记录；回复经本地验证后才关闭或回流为知识；
