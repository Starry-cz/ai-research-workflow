...
## 2026-08-09：停止后研究知识沉淀、检索与复用审计

### I-046：证据被保留，但未来成员仍无法按症状找到、判断和更新它

- **当前问题**：仓库已经保留运行命令、配置、日志、失败项、负结果和停止决定，但这些事实分散在实验卡、产物目录、Issue 与周会记录中。新手再次遇到同类症状时，仍可能从头排查；若把原配置、指标和日志复制到“经验库”，又会产生互相漂移的多份事实。现有流程也没有统一说明什么值得提炼、如何检索、复用前核对哪些身份与边界、复用后如何写回，以及旧经验何时应被替代或退役。
- **经验对照**：
  - [实验失败后，别只写“没做出来”](https://www.xiaohongshu.com/explore/6a3cb42f0000000021020e7b)指出只记录“效果不好”无法支持下次排查，应留下失败位置、预期差异、对照、改变条件和下一项检查；本仓库吸收“留下诊断线索”，不把个人实验记录方法提升为因果结论；
  - [关于用 Obsidian 做实验记录的感受](https://www.xiaohongshu.com/explore/69e1ac9d0000000023013230)及评论反映知识沉淀有前期配置、表格录入与工具学习成本，AI 可以汇总碎片但原始事实仍需留存；本仓库采用薄索引和原证据链接，不要求新手先搭建重型知识系统；
  - [研究生的实验记录该怎么记，以结果为导向](https://www.xiaohongshu.com/explore/696fa4fb000000001a02bc8e)反映流水账不利于复盘和汇报；本仓库按症状、诊断、适用范围与决定组织知识，但不删除完整运行记录；
  - 知乎文章[实验记录需要注意的关键点](https://zhuanlan.zhihu.com/p/534204566)以临时条件未记录导致长期无法复现为例，说明及时记录的重要性；其场景偏生物实验与合规，本仓库只吸收“改变必须可追溯”；
  - 知乎文章[如何科学地进行实验设计和记录](https://zhuanlan.zhihu.com/p/69353695)强调一次实验应回答问题并产生下一步；其中个人工具建议有时效性，本仓库吸收问题—证据—行动闭环，不指定统一笔记软件；
  - 知乎文章[上了电子实验记录本，之前的实验记录怎么办](https://zhuanlan.zhihu.com/p/2058238080742594121)反映历史记录迁移、原始记录可追溯和系统切换困难；文章带有商业 ELN 语境，只用于确认迁移痛点，不构成产品推荐。
- **规范与项目对照**：
  - [Architecture Decision Record](https://github.com/architecture-decision-record/architecture-decision-record)使用背景、决定、后果和状态记录重要选择，并通过 superseded 关系保留历史；本仓库借鉴状态链，但科研知识条目仍必须链接实验原证据；
  - [eLabFTW](https://github.com/elabftw/elabftw)提供可搜索、带权限和导入导出的电子实验记录；[The Turing Way 的 ELN 指南](https://book.the-turing-way.org/reproducible-research/rdm/rdm-elns/)同时提示成本、学习曲线、安全、导出与供应商锁定，因此服务器式 ELN 不是零基础的默认前提；
  - [DVC](https://github.com/iterative/dvc)和[MLflow Tracking](https://mlflow.org/docs/latest/ml/tracking/tracking-api/)可以版本化或检索参数、指标、产物、标签与代码身份；这些元数据帮助定位运行，不自动证明根因、适用范围或研究主张；
