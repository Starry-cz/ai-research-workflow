...
不要看见一篇热门论文就立即投入完整训练。先保留两到三个候选，区分“必须通过的准入条件”和“提高成功率的加分项”。

| 准入类型 | 检查内容 |
| --- | --- |
| **必须确认** | 能指出准备复现的具体表格、图或指标；论文版本稳定；代码与论文关系明确；数据可合法获得；训练和评测入口存在；所需资源不超过预算。 |
| **加分项** | 作者或官方组织维护；提供依赖锁定、预训练权重、精确命令、结果表、小数据示例、活跃 issue 和近期可用记录。 |
| **不能单独作为依据** | Star、榜单名次、论文新旧、宣传性 demo、他人转存的数据或未经核验的第三方复现。 |

#### 用低成本预检决定是否投入

使用[首篇真实 baseline 准入卡](templates/11-first-baseline-gate.md)，按成本从低到高检查：

```text
锁定目标结果与论文版本
  → 核对官方仓库、commit、许可证和数据入口
  → 找到依赖、训练、评测、权重与精确命令
  → 在干净环境完成安装和 import
  → 运行官方权重或最小公开样例
  → 用短试跑实测显存、吞吐与存储
...
- **PROCEED**：关键入口可访问，最小链路通过，完整复现预算可承受；
- **REFINE**：仍有可在限定时间内核验的缺口；写明负责人、截止时间和失败后的备选候选；
- **STOP**：数据无权使用、目标指标无法定位、关键训练或评测入口缺失、预算明显不可承受，或最小链路反复失败且原因不能收敛。

`STOP` 只表示“不适合作为你的首篇 baseline”，不评价论文质量。只有推理代码的项目可以用于功能演示或代码阅读，但不能把运行 demo 称为完整训练复现；只有第三方实现时，应明确来源风险，首篇复现优先切换到资料更完整的候选。

选择时同时保留一个备选候选，并预先写出四个里程碑：目标结果已定位、官方权重评测通过、短训练通过、完整协议达到预设容差。每个里程碑分别验收，不能用较低层级的成功替代较高层级。

**阶段验收**：能够展示已填写的准入卡，说明为什么选它、准备先运行什么、成功标准是什么、何时停止，以及停止后切换到哪个候选。
