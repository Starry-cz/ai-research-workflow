...
- 不把完成全部计算机课程作为开始科研的前提；
- 不把 Star 数量、排行榜或顶会标签等同于研究质量；
- 不推荐没有训练脚本、评测协议或许可证说明的项目作为首个复现对象；
- 不让 AI 编造论文、引用、代码能力、实验数据或审稿意见；
- 不绑定某个模型、Agent、云平台或商业服务。
...
- 不编造、篡改或选择性隐藏数据、代码、图表和实验结果；
- 不把没有运行过的实验写成已完成；
- 不允许 AI 在未审计的情况下修改 baseline、数据划分或评测协议；
- 所有引用都确认真实存在，且原文支持对应表述；
- 不上传未公开数据、受限数据、审稿材料、个人信息或保密文稿；
- 遵守论文、代码、数据集和模型的许可证与使用条款；
- AI 生成的文字、代码和图片必须由作者理解、修改和验证；
- “降低 AI 痕迹”不能替代真实性、证据质量和作者责任。
