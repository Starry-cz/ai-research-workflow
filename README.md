...
- **L0：不会终端、Git 或 Python 环境** → 先做[零基础准备检查表](templates/00-readiness-checklist.md)，再完成[开始科研前的最低能力](#开始科研前的最低能力)和八周路线第 1–3 周；
- **L1：能运行 Notebook，但没有复现过论文** → 建立[研究简报](templates/01-research-brief.md)和[文献检索账本](templates/07-literature-search-log.md)，从[首篇 baseline 筛选表](#首篇-baseline-筛选表)进入第 4–6 周；
