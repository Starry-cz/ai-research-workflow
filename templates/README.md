# 模板索引

按当前阶段复制模板到自己的研究仓库，不必一次使用全部模板。

| 模板 | 使用时机 | 完成标志 |
| --- | --- | --- |
| [00-readiness-checklist.md](00-readiness-checklist.md) | 开始科研前 | 找到当前最大能力缺口 |
| [01-research-brief.md](01-research-brief.md) | 选择方向和 baseline 时 | 问题、资源和风险边界明确 |
| [02-paper-reading-card.md](02-paper-reading-card.md) | 阅读核心论文时 | 方法与证据可以回到原文位置 |
| [03-reproduction-plan.md](03-reproduction-plan.md) | 克隆论文代码后 | 复现被拆成可验证关卡 |
| [04-experiment-card.md](04-experiment-card.md) | 每次运行正式实验前 | 唯一主要问题和停止条件明确 |
| [05-weekly-review.md](05-weekly-review.md) | 每周或每个阶段结束时 | 证据、失败、决策和下一步已记录 |

